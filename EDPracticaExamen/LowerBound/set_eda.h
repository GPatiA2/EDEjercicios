...
public:
   ...
   std::pair<bool,T> lower_bound(T const& e){
	   if(raiz == nullptr){
		   ...
		   return minimo(e, raiz);
	   }
   }

protected:

   std::pair<bool,T> minimo(T const & e, Link const& l){
	   if(l->elem < e){
		   return minimo(e, l->dr);
	   }
	   else if(l->elem > e){
		   T valorProvisional = l->elem;
		   std::pair<bool, T> aux = resolver(e, l->iz);
		   if(!aux.first){
			   return std::pair<bool,T>(true, valorProvisional);
		   }
		   else{
			   return aux;
		   }
