   		...
   		if(raiz == nullptr){
			...
			bintree<T> der = right();
   			...
   			if(izq.empty() && der.empty()){
   				...
   				tsol sD = der.calcular();
   				devolver.altura = sD.altura+1;
   				devolver.hojas = sD.hojas;
   				devolver.nodos = sD.nodos +1;
   				return devolver;
   			}
   			else if(der.empty() && !izq.empty()){
   				tsol sI = izq.calcular();
   				devolver.altura = sI.altura+1;
   				devolver.hojas = sI.hojas;
   				devolver.nodos = sI.nodos +1;
   				return devolver;
   			}
   			else{
   				tsol sD = der.calcular();
   				...
   				devolver.altura = max(sD.altura, sI.altura)+1;
   				devolver.hojas = sD.hojas + sI.hojas;
   				devolver.nodos = sD.nodos + sI.nodos + 1;
