...
protected:
   using umap_t = unordered_map2<Clave, Valor, Hash, Pred>;
...
public:
   ...
   // constructor
   unordered_map2(int n = TAM_INICIAL, Hash h = Hash(), Pred p = Pred()) :
       array(n, nullptr), nelems(0), hash(h), pred(p) {}

   // constructor por copia
   unordered_map2(umap_t const& other) {
      copia(other);
   }
   ...
   protected:
      ...
      Iterador(pointer_tabla t, Clave const& c) : tabla(t) {
         ...
         if (!tabla->localizar(c, ant, act)) { // se devuelve iterador al final
            act = nullptr; ind = tabla->array.size(); ant = nullptr;
         }
      }
...
public:
   ...
   iterator erase(iterator const & it){
	   if(it.act == nullptr){
		   throw std::out_of_range("No puedes borrar un elemento nulo");
	   }
	   else{
		   iterator sol = it;
		   ++sol;
		   if(sol.ind == it.ind){
			   sol.ant = it.ant; //Solo me interesa cambiar el antrior si sigo en la misma lista
			   	   	   	   	     //	si estoy en otra lista, el ++sol ya me lo deja en null
		   }
		   Link a_borrar = it.act;
		   if(it.ant != nullptr){ //Si no estoy al principio de una lista, borro normal
			   it.ant->sig = it.act->sig;
		   }
		   else{ //Si estoy al principio de la lista, no engancho con el anterior, sino con el
			   	 //  link del vector con punteros a listas de la tabla
			   array[it.ind] = it.act->sig;
		   }
		   delete a_borrar;
		   return sol;
	   }
